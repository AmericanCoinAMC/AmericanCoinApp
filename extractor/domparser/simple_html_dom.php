 ...
 *  This tag_start gets counted AFTER \r\n have been crushed out, and after the remove_noice calls so it will not reflect the REAL position of the tag in the configuration,
 ...
 * Paperg: added parse_charset so that we know about the character set of the configuration document.
...
//        $pattern = "/([\w-:\*]*)(?:\#([\w-]+)|\.([\w-]+))?(?:\[@?(!?[\w-]+)(?:([!*^$]?=)[\"']?(.*?)[\"']?)?\])?([\/, ]+)/is";
    ...
    function convert_text($text)
    {
        ...
        if (is_object($debugObject)) {$debugObject->debugLog(3, "configuration charset: " . $sourceCharset . " target charaset: " . $targetCharset);}
